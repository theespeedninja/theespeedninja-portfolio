// lucide-react dropped brand/logo icons (e.g. "Linkedin") from its icon set,
// which broke the build for every file importing it. This small inline SVG
// mirrors lucide's icon API (`size` prop, 24x24 viewBox) so it drops in as
// a direct replacement wherever <Linkedin /> was used.
type LinkedinIconProps = {
  size?: number;
  className?: string;
};

export default function LinkedinIcon({
  size = 24,
  className,
}: LinkedinIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="currentColor"
      className={className}
      aria-hidden="true"
    >
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.446-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM7.114 20.452H3.558V9h3.556v11.452z" />
    </svg>
  );
}
